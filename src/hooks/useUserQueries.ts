...
export const useLoginMutation = () => {
  ...
  return useMutation({
    ...
    onError: (
      error: Error & { response?: { data?: { result?: { mensaje?: string } } } }
    ) => {
...
export const useCompanyAreasQuery = () => {
  ...
  return useQuery({
    ...
    > => {
      const companyAreas = await (
        authApi as {
          getCompanyAreas: () => Promise<
            Array<{
              ID_EMPRESA: number;
              EMPRESA: string;
              LOGO: string | null;
      ...
      if (actualCompanyArea) {
        const stillExists = companyAreas.find(
          (ca) =>
            ca.ID_EMPRESA === actualCompanyArea!.ID_EMPRESA &&
            ca.ID_AREA === actualCompanyArea!.ID_AREA
        );
        if (!stillExists) {
      ...
      if (!actualCompanyArea) {
        ...
        const savedCompanyAreaIds = sessionStorage.getItem(
          "selected_company_area_ids"
        );
