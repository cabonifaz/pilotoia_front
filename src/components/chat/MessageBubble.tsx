...
export const MessageBubble = memo(
  ({
    ...
    // Normalize content: older messages use `message`, optimistic messages may use `content`
    const rawText = (message.message ??
      (message as any).content ??
      "") as string;
    const isTableOrList = hasTableOrList(rawText);

    // Remove <br> tags and add displaystyle to formulas
    const cleanContent = rawText.replace(/<br\s*\/?\s*>/gi, "");
    const withDisplayStyle = addDisplayStyle(cleanContent);
    const processedContent = isTableOrList
      ? fixTableMarkdown(withDisplayStyle)
      : withDisplayStyle;
      ...
      >
        <Card
          className={`max-w-[80%] border-0 shadow-none ${
            isUserMessage ? "bg-muted" : "bg-background"
          }`}
        >
          <CardHeaderCompact className="pb-2">
            <div className="flex items-center gap-2 text-xs">
              <Avatar className="h-6 w-6">
                <AvatarFallback className="text-xs">{getIcon()}</AvatarFallback>
              </Avatar>
          ...
          <CardContentCompact>
            <div className="overflow-hidden">
              ...
              {streamingMessageId === message.id && (
                <Loader2 className="inline-block ml-0.5 h-3 w-3 animate-spin" />
              )}
            </div>
          </CardContentCompact>
        </Card>
