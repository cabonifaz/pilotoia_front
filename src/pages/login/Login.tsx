...
const ENCRYPTION_KEY =
  import.meta.env.VITE_LOGIN_ENCRYPTION_KEY || "default-fallback-key";
...
export const LoginPage = () => {
  ...
  const { register, handleSubmit, errors, onSubmit, isLoading, setValue } =
    useAuth();
  ...
  useEffect(() => {
    ...
    const matchedCompany = refToUse
      ? companiesLogin.find((c: CompanyLogin) => c.SECRET_KEY === refToUse)
      : null;
  ...
  const handleCompanySelect = (companyName: string): void => {
    const selectedComp = companiesLogin.find(
      (c: CompanyLogin) => c.RAZON_SOCIAL === companyName,
    );
    if (selectedComp) {
  ...
  const handleLoginSubmit = async (formData: LoginFormData) => {
    ...
    if (rememberMe) {
      localStorage.setItem(
        ...
        encryptData(formData.usuario),
      );
      localStorage.setItem(
        REMEMBER_ME_PASSWORD_KEY,
        encryptData(formData.clave_acceso),
      );
    } else {
      ...
      {hasValidRef && (
        <div className="fixed bottom-8 right-8 z-50">
          <Button
            onClick={clearRef}
            variant="blue"
            className="rounded-full shadow-xl flex items-center gap-3 px-7 h-12 hover:scale-105 transition-all active:scale-95"
          >
