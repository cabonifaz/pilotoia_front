...
export const chatApi = {
  ...
  getMessagesByChat: async (
    ...
    if (last_evaluated_key)
      params.append("last_evaluated_key", JSON.stringify(last_evaluated_key));

    const url = `/v1/messages/chat${
      params.toString() ? `?${params.toString()}` : ""
    }`;

    const response = await apiClient.post<MessageListResponse>(url, {
      chat_id: chatId,
      company_id,
      area_id,
    });
